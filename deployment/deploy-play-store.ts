 ...
 *     which exchanges the GitHub OIDC token for short-lived GCP tokens.
...
export const PACKAGE_NAME = 'ch.rhosys.rail';
...
async function diagnoseError(err: unknown, client: PublisherClient): Promise<never> {
  ...
  if (status === 401 || status === 403) {
    ...
    if (msg.includes('iam.serviceAccounts.getAccessToken')) {
      ...
      process.stderr.write(`The CI runner's project path is not authorized to impersonate\n`);
      ...
      process.stderr.write(`    member             = "principalSet://iam.googleapis.com/\${pool}/attribute.repository/<org>/<repo>"\n`);
      process.stderr.write(`  }\n\n`);
